...
int main()
{
	...
	cout << "≈лементи черги: "; Print(L, Count(L, L), 0); cout << endl;

	Process(L, L, Count(L, L), 0, 0);
	cout << "–узультат: "; Print(L, Count(L, L), 0);
...
void Process(Elem*& L, Elem* first, int size, int i, int j)
{
	if (i < size - 1)
	{
		Insert(L, prev_elem(L, i, size, 0)); //добавленн€ у к≥нець списку зворотних елемент≥в
		return Process(L, first, size, i + 1, j);
	}

	if (j < size - 1)
	{
		Remove(L, L);//видаленн€ лишн≥х елемент≥в
